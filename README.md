...
### Gerar Documentação sem dependências: 
```bash
go tool swag init -g ./api/main.go
```

### Gerar Documentação com dependências: 
```bash
go tool swag init -g ./api/main.go --parserDependency --parseDependencyLevel 3
```

## Usando bibliotecas privadas

### Replace
Mudar {{{YOUR_PROJECT}}} no Dockerfile pelo projeto privado.

### Env para bibliotecas privadas
```bash
env GITHUB_TOKEN=hash_github
```

## 🛠️ Configuração do Projeto

Para uma melhor organização utilize o template [Projeto Gorote](https://github.com/go-gorote/temp):
```
/gorote
├── /api             	# Início do projeto instancia do fiber e configurações
├── /app             	# Pasta onde fica as aplicações do projeto
├── /docs             	# Pasta documentação swagger
